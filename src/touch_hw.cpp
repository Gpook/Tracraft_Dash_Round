...
bool begin() {
    ...
    // Тачпанель наклеена зеркально относительно оси X дисплея: свайп вправо
    // давал убывающий X, и экран уезжал навстречу пальцу. Приводим оси здесь,
    // а не в обработчике жестов, чтобы координаты касания были пригодны для
    // любого будущего использования — кнопок, попадания по виджету и прочего.
    //
    // Ось Y проверить пока нечем: жесты по вертикали не заведены. Если
    // вертикальные касания окажутся перевёрнутыми, второй аргумент — тот, что
    // нужно поменять. Строка [touch] в логе печатает координаты касания, по
    // ней это видно сразу.
    s_drv.setSwapXY(false);
    s_drv.setMirrorXY(true, false);
...
Point poll() {
    ...
    // Лог только в момент нажатия, а не каждый кадр: нужен для проверки
    // ориентации осей, и засорять им поток на 20 кадрах в секунду незачем.
    static bool wasPressed = false;
    if (pt.pressed && !wasPressed) {
        Serial.printf("[touch] касание x=%d y=%d\n", pt.x, pt.y);
    }
    wasPressed = pt.pressed;
