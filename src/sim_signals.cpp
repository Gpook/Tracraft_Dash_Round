...
/// участок, где ни один колокол не дотягивается: между T9 (65 с) и T1 (7 с
/// следующего круга) около 17 секунд, и на них машина уходит в отсечку в пятой.
constexpr float LAP_TIME = 78.0f;
...
constexpr Corner CORNERS[] = {
    {  7.0f, +1.00f, 0.92f, 2.2f },  // T1  правая шпилька после прямой
    { 13.5f, -0.55f, 0.45f, 2.8f },  // T2  быстрый левый
    { 19.0f, -0.95f, 0.85f, 2.0f },  // T3  левая шпилька
    { 27.0f, +0.45f, 0.35f, 4.5f },  // T4  длинный правый вираж
    { 36.0f, +0.90f, 0.72f, 1.5f },  // T5  шикана, правая
    { 39.0f, -0.90f, 0.72f, 1.5f },  // T6  шикана, левая
    { 47.0f, -0.60f, 0.55f, 3.5f },  // T7  затяжной левый
    { 57.0f, +1.00f, 0.95f, 2.0f },  // T8  самая тугая правая
    { 65.0f, +0.35f, 0.28f, 2.5f },  // T9  изгиб перед прямой
};
constexpr int N_CORNERS = sizeof(CORNERS) / sizeof(CORNERS[0]);

constexpr float V_MAX = 194.0f;   // км/ч на главной прямой
...
/// Скорость на отсечке в каждой передаче, км/ч. В пятой чуть ниже V_MAX,
/// поэтому на прямой обороты заходят за последний порог шифт-лайта (7200) и он
/// успевает моргнуть отсечкой.
constexpr float GEAR_TOP[] = { 58.0f, 88.0f, 122.0f, 158.0f, 195.0f };
...
int gearAt(float speed) {
    for (int i = 0; i < N_GEARS; ++i) {
        if (speed < GEAR_TOP[i] * 0.995f) return i;
    }
    return N_GEARS - 1;
}
...
float rpmAt(float speed) {
    return fmaxf(RPM_IDLE, RPM_REDLINE * (speed / GEAR_TOP[gearAt(speed)]));
}
...
constexpr float WARMUP_SEC = 95.0f;   // за сколько секунд шины выходят на режим
